...
public struct CodeScannerView: UIViewControllerRepresentable {
    ...
    public class ScannerViewController: UIViewController,UIImagePickerControllerDelegate,UINavigationControllerDelegate{
        ...
        override public func loadView() {
            ...
            label.text = "Du benutzt einen Simulator!"
            label.textAlignment = .center
            let button = UIButton()
            button.translatesAutoresizingMaskIntoConstraints = false
            button.setTitle("Drücke hier um ein QR-Code einzulesen", for: .normal)
    ...
    public class ScannerViewController: UIViewController {
        ...
        override public func viewWillAppear(_ animated: Bool) {
            ...
            let xPos = (view.frame.size.width / 2) - (200 / 2)
            let yPos = (view.frame.size.height / 2) - (200 / 2)
            let scanRect = CGRect(x: xPos, y: yPos, width: 200, height: 200)
            
            #if DEBUG
            let area = UIView()
            area.frame = scanRect
            area.layer.borderColor = UIColor.red.cgColor
            area.layer.borderWidth = 2.0
            view.addSubview(area)
            view.bringSubviewToFront(area)
            #endif
            
            let substrect = UIView()
            substrect.frame = self.view.bounds

            let targetMaskLayer = CAShapeLayer()
            let extendTrans: CGFloat = 50
            let rec = CGRect(x: (scanRect.origin.x - (extendTrans / 2)), y: (scanRect.origin.y - (extendTrans / 2)), width: scanRect.width + extendTrans, height: scanRect.height + extendTrans)
            let square = UIBezierPath(roundedRect: rec, cornerRadius: 45)
            ...
            substrect.layer.mask = targetMaskLayer
            substrect.layer.backgroundColor = UIColor.black.cgColor
            substrect.alpha = 0.35
            substrect.clipsToBounds = true
            
            view.addSubview(substrect)
            view.bringSubviewToFront(substrect)
